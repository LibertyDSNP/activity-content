...
const requireToString = (obj: unknown, name: string): string => {
  if (!isString(obj))
    throw new InvalidActivityContentError(`${name} is not a string`);
  return obj as string;
};

const requireToArray = (obj: unknown, name: string): Array<unknown> => {
  if (!Array.isArray(obj))
    throw new InvalidActivityContentError(`${name} is not an Array`);
  return obj as Array<unknown>;
};

const isValidHrefField = (obj: string): boolean =>
  obj.match(HREF_REGEX) !== null;

const isValidPublishedField = (obj: string): boolean =>
  obj.match(ISO8601_REGEX) !== null;

const isValidDurationField = (obj: string): boolean =>
  obj.match(DURATION_REGEX) !== null;

const hasAtLeastOneSupportedAudioMediaType = (
  obj: Array<ActivityContentAudioLink>
): boolean => {
...
const hasAtLeastOneSupportedImageMediaType = (
  obj: Array<ActivityContentImageLink>
): boolean => {
...
const hasAtLeastOneSupportedVideoMediaType = (
  obj: Array<ActivityContentVideoLink>
): boolean => {
...
const toActivityContentAttachment = (
  obj: unknown
): ActivityContentAttachment | undefined => {
  try {
...
const requireValidDimensions = (
  obj: Record<string, unknown>,
  name?: string
): boolean => {
  if (obj["height"] && !isNumber(obj["height"]))
    throw new InvalidActivityContentError(`${name} height is invalid`);
  if (obj["width"] && !isNumber(obj["width"]))
    throw new InvalidActivityContentError(`${name} width is invalid`);
  return true;
};

const requireValidDuration = (
  obj: Record<string, unknown>,
  contentType: string
): boolean => {
...
export const requireToActivityContentAttachment = (
  ...
  if (!isRecord(obj))
    throw new InvalidActivityContentError(
      invalidRecordMessage("ActivityContentAttachment")
    ); // this check is required for type checking to work
  if (!obj["type"])
    throw new InvalidActivityContentError(
      "ActivityContentAttachment type is invalid"
    );
  switch (obj["type"]) {
    case "Image":
    ...
    default:
      throw new InvalidActivityContentError(
        "unknown ActivityContentAttachment type: " + obj["type"]
      );
  }
};

const requireIsMediaContentLink = (
  obj: Record<string, unknown>,
  name: string
): boolean => {
  if (obj["type"] !== "Link")
...
const requireIsActivityContentAudioLink = (
  obj: unknown
): obj is ActivityContentAudioLink => {
  if (!isRecord(obj))
    throw new InvalidActivityContentError(
      invalidRecordMessage("ActivityContentAudioLink")
    );
  requireIsMediaContentLink(obj, "ActivityContentAudioLink");
  return true;
};

const requireIsActivityContentImageLink = (
  obj: unknown
): obj is ActivityContentImageLink => {
  if (!isRecord(obj))
    throw new InvalidActivityContentError(
      invalidRecordMessage("ActivityContentImageLink")
    );
...
const requireIsActivityContentVideoLink = (
  obj: unknown
): obj is ActivityContentVideoLink => {
  if (!isRecord(obj))
    throw new InvalidActivityContentError(
      invalidRecordMessage("ActivityContentVideoLink")
    );
...
const requireIsActivityContentLocation = (
  obj: unknown
): obj is ActivityContentLocation => {
  if (!isRecord(obj))
    throw new InvalidActivityContentError(
      invalidRecordMessage("ActivityContentLocation")
    );
  if (obj["type"] !== "Place")
    throw new InvalidActivityContentError(
      "ActivityContentLocation type is not 'Place'"
    );
  ...
  ["accuracy", "altitude", "latitude", "longitude", "radius"].forEach(
    (dimension) => {
      if (obj[dimension] && !isNumber(obj[dimension]))
        throw new InvalidActivityContentError(
          `ActivityContentLocation ${dimension} is not a number`
        );
    }
  );
...
const requireIsActivityContentTag = (
  obj: unknown
): obj is ActivityContentTag => {
  if (!isRecord(obj))
    throw new InvalidActivityContentError(
      invalidRecordMessage("ActivityContentTag")
    );
  if (obj["type"] === "Mention") {
...
const requireIsActivityContentHashtag = (
  obj: unknown
): obj is ActivityContentHashtag => {
  if (!isRecord(obj))
    throw new InvalidActivityContentError(
      invalidRecordMessage("ActivityContentHashtag")
    ); // this check is required for type checking to work
  requireToString(obj["name"], "ActivityContentHashtag name");
  return true;
};

const requireIsActivityContentMention = (
  obj: unknown
): obj is ActivityContentMention => {
  if (!isRecord(obj))
    throw new InvalidActivityContentError(
      invalidRecordMessage("ActivityContentMention")
    ); // this check is required for type checking to work
  if (!isDSNPUserURI(obj["id"]))
    throw new InvalidActivityContentError(
      "ActivityContentMention id is not a valid DSNPUserURI"
    );
...
const requireToActivityContentAudio = (obj: unknown): ActivityContentAudio => {
  ...
  if (!isRecord(obj))
    throw new InvalidActivityContentError(invalidRecordMessage(objectName)); // this check is required for type checking to work
  if (obj["name"]) requireToString(obj["name"], objectName + " name");
  if (obj["duration"]) requireValidDuration(obj, objectName);

  if (obj["type"] !== "Audio")
    throw new InvalidActivityContentError(`${objectName} type is not "Audio"`);
  ...
  if (!hasAtLeastOneSupportedAudioMediaType(obj["url"]))
    throw new InvalidActivityContentError(
      objectName + " has no supported media types"
    );
...
const requireToActivityContentImage = (obj: unknown): ActivityContentImage => {
  ...
  if (obj["type"] !== "Image")
    throw new InvalidActivityContentError(`${objectName} type is not "Image"`);
...
const requireToActivityContentVideo = (obj: unknown): ActivityContentVideo => {
  ...
  if (obj["type"] !== "Video")
    throw new InvalidActivityContentError("type is not 'Video'");
...
const requireIsActivityContentBaseType = (
  obj: unknown,
  typeName: string
): boolean => {
  if (!isRecord(obj))
    throw new InvalidActivityContentError("ActivityContentNote"); // this check is required for type checking to work
  if (obj["@context"] !== "https://www.w3.org/ns/activitystreams")
    throw new InvalidActivityContentError(
      `invalid ActivityContent${typeName} @context`
    );
  if (obj["type"] !== typeName)
    throw new InvalidActivityContentError(
      `invalid ActivityContent${typeName} type`
    );
  if (obj["name"])
    requireToString(obj["name"], `ActivityContent${typeName} name`);
  if (obj["published"])
    requireToString(obj["published"], `ActivityContent${typeName} published`);
  if (obj["tag"] && !isRecord(obj["tag"]))
    requireToArray(obj["tag"], `ActivityContent${typeName} tag`);
  if (obj["location"] && !isRecord(obj["location"]))
    throw new InvalidActivityContentError(
      invalidRecordMessage(`ActivityContent${typeName} location`)
    );
  return true;
};
 ...
 */
export const requireIsActivityContentNoteType = (
  obj: unknown
): obj is ActivityContentNote => {
  if (!isRecord(obj))
    throw new InvalidActivityContentError("ActivityContentNote"); // this check is required for type checking to work
  requireIsActivityContentBaseType(obj, "Note");
  if (obj["mediaType"] !== "text/plain")
    throw new InvalidActivityContentError(
      "invalid ActivityContentNote mediaType"
    );
  if (obj["attachment"])
 ...
 */
export const isActivityContentNoteType = (
  obj: unknown
): obj is ActivityContentNote => {
 ...
 * @throws InvalidActivityContentError if type checks fail.
 */
export const requireIsActivityContentProfileType = (
  obj: unknown
): obj is ActivityContentProfile => {
  if (!isRecord(obj))
    throw new InvalidActivityContentError(
      invalidRecordMessage("ActivityContentProfile")
    );
  ...
  if (
    obj["icon"] &&
    !isArrayOfType(obj["icon"], requireIsActivityContentImageLink)
  )
    throw new InvalidActivityContentError(
      "ActivityContentProfile icon is invalid"
    );
  if (obj["attachment"])
 ...
 */
export const isActivityContentProfileType = (
  obj: unknown
): obj is ActivityContentProfile => {
  if (!isRecord(obj)) return false;
  try {
 ...
 *    returns just #2.
 */
export const requireGetSupportedContentAttachments = (
  obj: Array<unknown>
): Array<ActivityContentAttachment> => {
...
export const requireValidActivityContentNote = (obj: unknown): boolean => {
  if (requireIsActivityContentNoteType(obj)) {
    ...
    if (obj["published"] && !isValidPublishedField(obj["published"]))
      throw new InvalidActivityContentError(
        "ActivityContentNote published is invalid"
      );
    if (obj["attachment"]) {
      const attachments = requireToArray(
        obj["attachment"],
        "ActivityContentNote attachment"
      );
    ...
    throw new InvalidActivityContentError(
      "ActivityContentNote is invalid for an unknown reason"
    );
  }
  return true;
};
...
export const requireValidActivityContentProfile = (obj: unknown): boolean => {
  if (requireIsActivityContentProfileType(obj)) {
    if (obj["tag"] && !isArrayOfType(obj["tag"], requireIsActivityContentTag))
      requireIsActivityContentTag(obj["tag"]);

    if (obj["published"] && !isValidPublishedField(obj["published"]))
      throw new InvalidActivityContentError(
        "ActivityContentProfile published is invalid"
      );
    if (obj["icon"]) {
    ...
    throw new InvalidActivityContentError(
      "ActivityContentProfile is invalid for an unknown reason"
    );
  }
  return true;
};
